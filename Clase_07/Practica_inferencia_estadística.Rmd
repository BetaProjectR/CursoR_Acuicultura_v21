...
Corresponde a la aplicación de diferentes métodos que permiten obtener o extraer conclusiones sobre los parámetros de una población a partir de una muestra de datos tomada en ella.

También es posible calcular el grado de fiabilidad (probabilidad) de que los resultados extraídos de un estudio son verdaderos o falsos.
...
**2**. - Realizar análisis de inferencia estadística para comparación datos cuantitativos continuos. 
...
|  **Librería** | **Función**      |
|:-------------|:------------------|
| **ggplot2** |  Permite la visualización de datos preferido para realizar graficas avanzadas con R Hoja de referencia.|
...
| **stats**| Posee funciones para hacer las pruebas de hipótesis para la media y diferencia de medias, con muestras independientes y dependientes (o pareadas). También, para hacer la prueba de hipótesis de la correlación. |
...
**3.-** Realiza la prueba de hipotesis para la correlación entre x e y: **cor.test()**
...
**6.-** Realiza la prueba de hipotesis para la media y diferencia de medias, con muestras independientes y dependientes (o pareadas): **t.test()**

**7.-** Extrae los intervalos de confianza para la media y diferencia de medias, con muestras independientes y dependientes (o pareadas): **t.test()$conf.int**
...
**d).** ¿Es esta correlacción estadísticamente distinta de cero?. Escriba su hipótesis nula y alternativa para este experimento.
...
**a).** Importe el set de datos **Salmones_Machos.xlsx** usando la función **read_excel()** de la librería **readxl**, llame al objeto Salmones. Explore el set de datos usando las funciones **head()**, **str()** y **summary()**. 
...
**e).** Realice una prueba de hipótesis para el peso promedio de los machos usando la función **t.test()**, (llame al objecto DifMed_M) y obtenga los intervalos de confianza usando **t.test()$conf.int** (llame a este objeto IC_DifMed_M) . Considere en **t.test** como nivel de confianza para la prueba el 95% y los parámetros, **alternative = c("greater")**, **paired=FALSE, var.equal=FALSE**. Para mostrar los resultados de la prueba de diferencia de medias usar la función **pander()** sobre el objeto **DifMed_M**, en el parámetro **_caption_** de la función **pander()** colocar "Prueba de hipótesis para el peso promedio de machos."
